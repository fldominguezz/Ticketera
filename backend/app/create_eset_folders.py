...
async def run():
    ...
    async with AsyncSessionLocal() as db:
        print(f"Iniciando creación de {len(final_paths)} jerarquías...")
        count = 0
        for path in sorted(final_paths):
            ...
            try:
                ...
                if count % 20 == 0:
                    await db.commit()
                    print(f"Creadas/Verificadas {count} rutas...")
            except Exception as e:
                print(f"Error en ruta {target_path}: {e}")
        
        await db.commit()
        print(f"PROCESO COMPLETADO: {count} rutas procesadas exitosamente.")
