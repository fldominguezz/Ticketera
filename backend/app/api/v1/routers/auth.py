...
async def authenticate_user_local(
    ...
    if not user.is_active:
        print(f"DEBUG LOGIN: User '{identifier}' is inactive")
        return None

    # Check if locked
    if user.locked_until and user.locked_until > datetime.now(user.locked_until.tzinfo):
        print(f"DEBUG LOGIN: User '{identifier}' is LOCKED until {user.locked_until}")
    ...
    if not verify_password(password, user.hashed_password):
        print(f"DEBUG LOGIN: Password verification FAILED for user '{identifier}'")
    ...
    print(f"DEBUG LOGIN: Password verification SUCCESS for user '{identifier}'")
...
async def login(
    ...
    """
    print(f"DEBUG LOGIN ATTEMPT: identifier='{login_data.identifier}' IP='{request.client.host}'")
