    ...
    async with AsyncSessionLocal() as session:
        ...
        for loc in locations:
            ...
            if match:
                ...
                print(f"Migrating: '{loc.name}' -> Dep: '{dep_name}', Code: '{dep_code}'")
                ...
                print(f"No code in: '{loc.name}', setting name as dependency.")
                await session.execute(
                    update(Asset)
                    .where(Asset.location_node_id == loc.id)
        ...
        await session.commit()
        print("Disconnected assets and cleared location history.")
        ...
        await session.commit()
        print("All location folders deleted.")
