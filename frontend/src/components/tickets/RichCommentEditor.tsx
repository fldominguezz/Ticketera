    ...
    <Card className="bg-black bg-opacity-20 border-white border-opacity-10 overflow-visible mb-4 position-relative">
      <div className="p-0">
        <Form.Control 
          ...
          className="bg-transparent border-0 text-white shadow-none p-3"
          style={{ resize: 'none', fontSize: '14px', minHeight: '100px' }}
        />
      </div>

      {showMentions && filteredUsers.length > 0 && (
        <Card className="position-absolute shadow-lg border border-primary border-opacity-25 bg-dark text-white z-3" style={{ bottom: '100%', left: '10px', width: '250px', marginBottom: '5px' }}>
          <ListGroup variant="flush">
            {filteredUsers.map(u => (
              <ListGroup.Item 
                ...
                className="bg-dark text-white border-white border-opacity-5 hover-bg-primary d-flex align-items-center gap-2 py-2"
              >
                <div className="bg-primary bg-opacity-20 p-1 rounded-circle"><User size={14} className="text-primary"/></div>
      ...
      <div className="p-2 bg-black bg-opacity-40 d-flex justify-content-between align-items-center border-top border-white border-opacity-10">
        <Form.Check 
