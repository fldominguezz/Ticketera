...
const renderWidgetContent = (w: Widget, stats: any) => {
    console.log("[renderWidgetContent] Widget:", w, "Stats:", stats);
    ...
    if (w.data_source === 'tickets_count') {
        console.log("[renderWidgetContent] Rendering tickets_count widget");
        content = (
        ...
        console.log("[renderWidgetContent] Rendering siem_alerts widget");
        ...
        console.log("[renderWidgetContent] Rendering default placeholder for:", w.data_source);
    ...
    console.log(`[renderWidgetContent] Returning content for widget ${w.id} (${w.data_source}):`, content);
    return content;
};
...
const DynamicDashboard: React.FC<Props> = ({ initialLayout, onSave, stats }) => {
  ...
  useEffect(() => {
    console.log("[DynamicDashboard] useEffect triggered. initialLayout:", initialLayout);
    console.log("[DynamicDashboard] Stats received in useEffect:", stats);
    setMounted(true);
    if (!Array.isArray(initialLayout) || initialLayout.length === 0) {
      console.log("[DynamicDashboard] Using default layout because initialLayout is empty");
      setLayout([
      ...
      console.log("[DynamicDashboard] Setting layout from initialLayout:", sanitized);
      setLayout(sanitized);
    }
  }, [initialLayout]);

  console.log("[DynamicDashboard] Rendering with layout:", layout);
  console.log("[DynamicDashboard] Rendering with stats:", stats);
  console.log("[DynamicDashboard] isEditMode:", isEditMode);
  console.log("[DynamicDashboard] canEdit:", canEdit);

  const handleReset = async () => {
    if (confirm("¿Deseas restaurar el diseño de fábrica del dashboard?")) {
      try {
        const res = await api.post('/dashboard/reset');
        console.log("[DynamicDashboard] Reset response:", res.data);
        ...
        {layout.map(w => (
          <div key={w.id} className="widget-container">
            <Card className="h-100 shadow-sm widget-card">
              <div className={`p-2 px-3 d-flex justify-content-between align-items-center ${isEditMode ? 'bg-primary text-white' : 'bg-primary bg-opacity-10'}`}>
                <div className="d-flex align-items-center gap-2">
                   {isEditMode && <GripVertical size={14} className="widget-drag-handle cursor-move" />}
                   <span className={`x-small fw-black uppercase tracking-widest ${isEditMode ? 'text-white' : 'text-primary'}`}>{w.title}</span>
                </div>
