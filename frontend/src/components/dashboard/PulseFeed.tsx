...
export const PulseFeed: React.FC = () => {
  ...
  return (
    <Card className="bg-black bg-opacity-40 border-0 shadow-lg h-100 overflow-hidden">
      <div className="p-3 bg-primary bg-opacity-10 border-bottom border-white border-opacity-5 d-flex justify-content-between align-items-center">
      ...
      <Card.Body className="p-0 overflow-auto" style={{ maxHeight: '400px' }}>
        {events.map((ev, i) => (
          <div key={ev.id || i} className="p-3 border-bottom border-white border-opacity-5 hover-bg-light transition-all">
            <div className="d-flex justify-content-between mb-1">
              <div className="d-flex align-items-center gap-2">
                {getEventIcon(ev.event_type)}
                <span className="x-small fw-black text-white uppercase" style={{fontSize: '9px'}}>
