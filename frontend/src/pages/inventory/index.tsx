...
import { 
  ...
  XCircle, AlertTriangle, UploadCloud, Search, Plus
} from 'lucide-react';
import { 
...
export default function InventoryPage() {
  ...
  }, [showDecommissioned, debouncedSearch]);
  ...
  const confirmImport = async () => {
    ...
    try {
        const res = await api.post(`/assets/import?source=${importSource}`, importData);
  ...
  const filteredAssets = assets.filter(asset => {
    ...
    if (depFilter && !asset.dependencia?.toLowerCase().includes(depFilter.toLowerCase())) return false;
    if (codeFilter && !asset.codigo_dependencia?.includes(codeFilter)) return false;
    return true;
  });
            ...
            <Card.Body className="py-2 px-3 d-flex flex-wrap align-items-center justify-content-between gap-3">
                <div className="d-flex align-items-center flex-grow-1 position-relative" style={{ maxWidth: '500px' }}>
                    <InputGroup className="border rounded-pill px-3 py-1 bg-light">
                        ...
                        />
                        {searchTerm && (
                          <Button variant="link" className="p-0 text-muted" onClick={() => setSearchTerm('')}><XCircle size={16} /></Button>
                        )}
                    </InputGroup>
                </div>
                ...
                <div className="table-responsive custom-scrollbar">
                    ...
                    <thead className="bg-light">
                        ...
                        <tr className="bg-white border-bottom">
                            ...
                            <td className="py-1">
                                <Form.Control 
                                    size="sm" placeholder="Filtrar dep..." 
                                    className="x-small fw-bold border-0 bg-light"
                                    value={depFilter} onChange={e => setDepFilter(e.target.value)}
                                />
                            </td>
                            <td className="py-1">
                                <Form.Control 
                                    size="sm" placeholder="Num..." 
                                    className="x-small fw-bold border-0 bg-light"
                                    type="number"
                    ...
                    <tbody>
                        ...
                        <tr><td colSpan={8} className="text-center py-5 text-muted x-small fw-bold">NO SE ENCONTRARON DISPOSITIVOS</td></tr>
                        ...
                        <tr key={asset.id} className={`border-bottom hover-row ${selectedAssets.has(asset.id) ? 'bg-primary bg-opacity-5' : ''}`}>
                            ...
                            <td className="small fw-bold text-uppercase text-truncate">{asset.dependencia || '---'}</td>
                            <td className="small font-monospace">{asset.codigo_dependencia || '---'}</td>
