...
export default function AuditLogsPage() {
  ...
  const getEventBadge = (type: string) => {
    ...
    if (l.includes('update') || l.includes('reassigned')) return <Badge bg="warning" className="px-2 py-1 x-small fw-black uppercase text-dark">{type}</Badge>;
                    ...
                    <tr key={log.id} className="border-bottom border-color">
                      ...
                      <td>
                        <div className="d-flex align-items-center gap-2">
                           <div className="avatar-xs">{log.user?.username?.charAt(0).toUpperCase() || 'S'}</div>
                           <span className="small fw-black text-main">{log.user?.username || 'SISTEMA'}</span>
                        </div>
                      </td>
